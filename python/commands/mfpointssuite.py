...
def setup(tree: app_commands.CommandTree):
    ...
    async def nco(intact: Interaction, user: Member, category: app_commands.Choice[int], addsub:app_commands.Choice[int], amount: float):
        try:
            ...
            i = await intact.client.get_guild(926850392271241226).get_channel(1308928443974684713).send(embed=Embed(title=f"[Error][Points {inspect.currentframe().f_code.co_name}]", description=format_exc(2)))
